///======================MACRO=========================///
macro_name = "Cell_RoiMaker_timelapse";
///====================================================///
///File author(s): Stéphane Verger=====================///

///====================Description=====================///
/*This macro allows a semi-automated segmentation and 
creation of ROI sets for 2D timelapse series.
It is modified from the simple Cell_RoiMaker, to create 
ROI set for time series. New ROI do not have to be created 
for each image in a time serie.
See https://github.com/VergerLab/MT_Angle2Ablation_Workflow 
for more detailed explanations of use.
*/
macro_source = "https://github.com/VergerLab/MT_Angle2Ablation_Workflow/Cell_RoiMaker_timelapse.ijm";
...
for (j=0; j<list.length; j++){
	...
	if(endsWith (list[j], contours_suffix)){
		...
		if(segmentation_done == false){
			...
			// clean up the ROI manager
			roiManager("reset");
			//Open roiset from previous processed image
			roiManager("Open", dir + File.separator + Output_ROIs);
		}
