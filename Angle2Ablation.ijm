...
///====================Description=====================///
/*This macro allows an automated calculation of angles between the output 
of fibrilToolBatch (or other) and manually drawn lines corresponding to an ablation.
It natively uses as input, output from the fibriltoolbatch.ijm macro as well as from 
the Line_RoiMaker.ijm macro.
*/
...
for (j=0; j<list.length; j++){
	...
	if(endsWith (list[j], microtubule_image_suffix)){
		...
		for (i = 0; i < ft_roi_number; i++) {
		    ...
		    FT_X = getResult("X", i);
		    FT_Y = getResult("Y", i);
		    Line_X = getResult("X", i + ft_roi_number);
		    Line_Y = getResult("Y", i + ft_roi_number);
		    setColor("blue");
			...
			Input_anisotropy = File_name + "_FT.txt";
